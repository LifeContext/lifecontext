<template>
  <div 
    ...
    ref="todoContainer"
  >
    <h2 class="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4">{{ t('todo.title') }}</h2>
    ...
    <div v-else class="flex flex-col h-full min-h-0">
      <div class="mb-4 flex-shrink-0">
        <div class="flex justify-between text-sm mb-1">
          <span class="text-slate-600 dark:text-slate-400">{{ t('todo.progressLabel') }}</span>
          <span class="font-medium text-slate-700 dark:text-slate-300">{{ progressSummary }}</span>
        </div>
      ...
      <div class="space-y-2 flex-1 overflow-y-auto min-h-0" ref="listContainer">
        <!-- 空状态 -->
        <div v-if="todos.length === 0" class="py-8 flex items-center justify-center">
          <div class="text-center">
            <div class="text-slate-300 dark:text-slate-600 mb-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="mx-auto" viewBox="0 0 16 16">
            ...
            <p class="text-sm text-slate-400 dark:text-slate-500">{{ t('todo.emptyTitle') }}</p>
            <p class="text-xs text-slate-300 dark:text-slate-500 mt-1">{{ t('todo.emptySubtitle') }}</p>
          </div>
        </div>
        ...
        <template v-for="(todo, index) in displayTodos" :key="todo.id">
          ...
          >
            ...
            <span v-if="editingId !== todo.id" :class="`text-xs px-2 py-1 rounded ${ 
              ...
              {{ getPriorityBadgeLabel(todo.priority) }}
            </span>
      ...
      <div class="flex-shrink-0 pt-4 border-t border-slate-300 dark:border-slate-600 mt-auto">
        <div class="flex items-center gap-2">
          ...
          >
            <option value="low">{{ t('todo.priority.lowPriority') }}</option>
            <option value="medium">{{ t('todo.priority.mediumPriority') }}</option>
            <option value="high">{{ t('todo.priority.highPriority') }}</option>
          </select>
          <button
            @click="addTodo"
      ...
      <div v-if="showDeleteConfirm" class="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
        <div class="bg-white dark:bg-slate-800 rounded-lg p-4 shadow-xl">
          <p class="text-slate-700 dark:text-slate-200 mb-4">{{ t('todo.deleteConfirm') }}</p>
          <div class="flex justify-end gap-2">
            ...
            >
              {{ t('todo.buttons.cancel') }}
            </button>
            <button
              @click="deleteTodo"
              class="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600"
            >
              {{ t('todo.buttons.delete') }}
            </button>
          </div>
        </div>
      </div>
...
const getPriorityBadgeLabel = (priority?: number) => {
  if (priority === 3) return t('todo.priority.high');
  if (priority === 2) return t('todo.priority.medium');
